package bits.lottery.app.config;
...
import bits.lottery.model.Lottery;
import bits.lottery.properties.BitsLotteryProperties;
import bits.lottery.service.BitsLotteryService;
import okhttp3.OkHttpClient;

@Configuration
public class BitsLotteryConfiguration {

    private static final Logger LOG = LoggerFactory.getLogger(BitsLotteryConfiguration.class);
    ...
    @Autowired
    private BitsLotteryProperties config;
    ...
    @Bean
    public BitsLotteryService contract(Web3j web3j, @Value("${lottery.contract.address:}") String contractAddress)
            throws Exception {
    ...
    private BitsLotteryService initLotteryService(String contractAddress, Web3j web3j) {
        return new BitsLotteryService(contractAddress, web3j, config);
    }
